...
function getFormattedText(roles, text) {
	const phrases = text.split('\n');
	let output = '';
	roles.forEach(role => {
		output += (`${role}:\n`);
		phrases.forEach((phrase, j) => {
			if (phrase.match(new RegExp(`^${role}`))) {
				output += (`${j+1}) ${phrase.replace(new RegExp(`^${role}.`), '').trim()}\n`);
			}
		});
		output += '\n';
	});
...
console.log(getFormattedText(roles, text));